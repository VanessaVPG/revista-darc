<template>
  <div id="revista-darc">
    <header class="d-flex justify-content-between container pt-5">
      <div class="astronauta d-flex align-items-center">
        <b-img src="~/assets/img/astronauta-girl-power.svg"></b-img>
        <div>
          <h1 class="p-0 m-0">Vamos viajar?</h1>
          <p class="p-0 m-0">Escolha seu primeiro caminho</p>
        </div>
      </div>
      <div class="logo-sections">
        <LogoSections />
      </div>
    </header>
    <main class="container">
      <section id="menu" class="d-flex mx-auto">
        <div id="menu-mulheres">
          <NuxtLink to="#mulheres-importantes"
            ><div
              class="
                mulheres-importantes
                d-flex
                justify-content-center
                align-items-center
              "
            >
              <h2 class="text-center">
                MULHERES<br /><span>IMPORTANTES</span>
              </h2>
              <div class="shadow m-shadow position-absolute"></div></div
          ></NuxtLink>
        </div>
        <div id="menu-linha-do-tempo">
          <div
            class="
              linha-do-tempo
              d-flex
              justify-content-center
              align-items-center
            "
          >
            <h2>LINHA DO<span>TEMPO</span></h2>
            <div class="shadow l-shadow position-absolute"></div>
          </div>
        </div>
        <div id="menu-voce-ja-viu">
          <NuxtLink to="#voce-ja-viu">
            <div
              class="
                voce-ja-viu
                d-flex
                justify-content-center
                align-items-center
              "
            >
              <h2>
                VOCÊ JÁ <br /><span class="d-flex justify-content-center"
                  >VIU?</span
                >
              </h2>
              <div class="shadow v-shadow position-absolute"></div>
            </div>
          </NuxtLink>
        </div>
        <div id="menu-jogo-e-mundo">
          <div
            class="
              jogo-e-mundo
              d-flex
              justify-content-center
              align-items-center
            "
          >
            <h2>JOGO & <span>MUNDO</span></h2>
            <div class="shadow j-shadow position-absolute"></div>
          </div>
        </div>
      </section>
      <BotaoVoltar />

      <section id="mulheres-importantes">
        <div class="header d-flex justify-content-between">
          <LogoSections />
          <b-nav class="menu"> </b-nav>
        </div>
        <Flicking :options="{ circular: true }" :plugins="plugins">
          <div class="mulheres mr-5">
            <b-img src="~/assets/img/joana.svg"></b-img>
            <div class="shadow-foto position-absolute"></div>
          </div>
          <div class="flicking-panel" style="max-width: 300px; height: auto">
            <h1>Joana d'Arc</h1>
            <p class="text-justify mr-5">
              Não existem muitos dados sobre a vida da esposa de Zumbi dos
              Palmares. Sua figura é envolta de incertezas. Dizem que viveu no
              Brasil Colonial e foi descrita como uma guerreira valente, que
              dominava técnicas de capoeira e teria lutado muitas batalhas para
              preservar o Quilombo dos Palmares. Inclusive teria auxiliado Zumbi
              em estratégias e planos de ataque e defesa.
            </p>
            <p class="text-justify mr-5">
              Não existem muitos dados sobre a vida da esposa de Zumbi dos
              Palmares. Sua figura é envolta de incertezas. Dizem que viveu no
              Brasil Colonial e foi descrita como uma guerreira valente, que
              dominava técnicas de capoeira e teria lutado muitas batalhas para
              preservar o Quilombo dos Palmares. Inclusive teria auxiliado Zumbi
              em estratégias e planos de ataque e defesa.
            </p>
            <p class="text-justify mr-5">
              Não existem muitos dados sobre a vida da esposa de Zumbi dos
              Palmares. Sua figura é envolta de incertezas. Dizem que viveu no
              Brasil Colonial e foi descrita como uma guerreira valente, que
              dominava técnicas de capoeira e teria lutado muitas batalhas para
              preservar o Quilombo dos Palmares. Inclusive teria auxiliado Zumbi
              em estratégias e planos de ataque e defesa.
            </p>
          </div>
          <div class="mulheres mr-5">
            <b-img src="~/assets/img/joana.svg"></b-img>
            <div class="shadow-foto position-absolute"></div>
          </div>
          <div class="flicking-panel" style="max-width: 300px; height: auto">
            <h1>Joana d'Arc</h1>
            <p class="text-justify">
              Não existem muitos dados sobre a vida da esposa de Zumbi dos
              Palmares. Sua figura é envolta de incertezas. Dizem que viveu no
              Brasil Colonial e foi descrita como uma guerreira valente, que
              dominava técnicas de capoeira e teria lutado muitas batalhas para
              preservar o Quilombo dos Palmares. Inclusive teria auxiliado Zumbi
              em estratégias e planos de ataque e defesa.
            </p>
          </div>
          <span slot="viewport" class="flicking-arrow-prev is-circle"></span>
          <span slot="viewport" class="flicking-arrow-next is-circle"></span>
        </Flicking>
        <!--  <b-carousel
          id="carousel-1"
          v-model="slide"
          :interval="15000"
          controls
          indicators
          background="#000000"
          img-width="1024"
          img-height="480"
          style="text-shadow: 1px 1px 2px #333"
          @sliding-start="onSlideStart"
          @sliding-end="onSlideEnd"
        >
          <b-carousel-slide
            caption="Joana d'Arc"
            img-blank
            img-alt="Blank image"
          >
            <div class="d-flex">
              <div class="mulheres">
                <b-img src="~/assets/img/joana.svg"></b-img>
                <div class="shadow-foto position-absolute"></div>
              </div>
              <p class="text-justify">
                Não existem muitos dados sobre a vida da esposa de Zumbi dos
                Palmares. Sua figura é envolta de incertezas. Dizem que viveu no
                Brasil Colonial e foi descrita como uma guerreira valente, que
                dominava técnicas de capoeira e teria lutado muitas batalhas
                para preservar o Quilombo dos Palmares. Inclusive teria
                auxiliado Zumbi em estratégias e planos de ataque e defesa.
              </p>
            </div> -->

        <!--    </b-carousel-slide>
          <b-carousel-slide
            caption="Joana d'Arc"
            img-blank
            img-alt="Blank image"
          >
            <div class="d-flex">
              <div class="mulheres">
                <b-img src="~/assets/img/joana.svg"></b-img>
                <div class="shadow-foto position-absolute"></div>
              </div>
              <p class="text-justify">
                Não existem muitos dados sobre a vida da esposa de Zumbi dos
                Palmares. Sua figura é envolta de incertezas. Dizem que viveu no
                Brasil Colonial e foi descrita como uma guerreira valente, que
                dominava técnicas de capoeira e teria lutado muitas batalhas
                para preservar o Quilombo dos Palmares. Inclusive teria
                auxiliado Zumbi em estratégias e planos de ataque e defesa.
              </p>
            </div>
          </b-carousel-slide>
        </b-carousel> -->
      </section>
      <BotaoVoltar class="my-5" />
      <section id="voce-ja-viu">
        <div class="header d-flex justify-content-between">
          <LogoSections />
          <b-nav class="menu"> </b-nav>
        </div>
        <h1 class="vc-ja-viu-titulo text-right">
          <span class="bg">Filmes</span> e <span>séries</span> que<br />
          fazem a <span class="circulo">diferença</span>
        </h1>
        <Flicking
          :options="{ circular: true }"
          :plugins="plugins2"
          class="pb-5"
        >
          <div class="conteudo">
            <b-img src="~/assets/img/radioative.png" width="300px"></b-img>
            <div class="shadow-filmes position-absolute"></div>
          </div>
          <div style="max-width: 400px">
            <h1>Radioatividade</h1>
            <p>
              Não existem muitos dados sobre a vida da esposa de Zumbi dos
              Palmares. Sua figura é envolta de incertezas. Dizem que viveu no
              Brasil Colonial e foi descrita como uma guerreira valente, que
              dominava técnicas de capoeira e teria lutado muitas batalhas para
              preservar o Quilombo dos Palmares. Inclusive teria auxiliado Zumbi
              em estratégias e planos de ataque e defesa.
            </p>
          </div>

          <span slot="viewport" class="flicking-arrow-prev is-circle"></span>
          <span slot="viewport" class="flicking-arrow-next is-circle"></span>
        </Flicking>
        <BotaoVoltar class="my-5" />
        <div class="imagem-fundo-musicas">
          <div class="libras d-flex justify-content-end">
            <b-img src="~/assets/img/libras-estatico.png"></b-img>
          </div>
          <div class="titulo-musica-delas d-flex justify-content-end">
            <h1 class="text-right font-weight-bolder">
              Músicas<br />
              Delas
            </h1>
          </div>
          <div style="width:600px; margin-inline:auto;">
            <iframe
              style="border-radius: 12px;"
              src="https://open.spotify.com/embed/playlist/16FLAInUWEtgNvKuek7A9l?utm_source=generator&theme=0"
              width="100%"
              height="380"
              frameBorder="0"
              allowfullscreen=""
              allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture"
            ></iframe>
          </div>
        </div>
      </section>
    </main>
  </div>
</template>

<script>
import { Flicking } from "@egjs/vue-flicking";
import LogoSections from "~/components/LogoSections.vue";
import BotaoVoltar from "../components/BotaoVoltar.vue";
import { Arrow } from "@egjs/flicking-plugins";
import "@egjs/flicking-plugins/dist/arrow.css";
import { Perspective } from "@egjs/flicking-plugins";

export default {
  components: { LogoSections, BotaoVoltar, Flicking: Flicking },
  data() {
    return {
      plugins: [
        new Arrow(),
        new Perspective({ rotate: -1, scale: 2, perspective: 800 }),
      ],
      plugins2: [
        new Arrow(),
        new Perspective({ rotate: -1, scale: 2, perspective: 800 }),
      ],
    };
  },
};
</script>
<style scoped>
@keyframes grow-shrink {
  50% {
    transform: scale(1.2);
  }
}

@keyframes expand {
  50% {
    word-spacing: 20px;
  }
}
@keyframes translate {
  0% {
    transform: translateX(233px);
  }
  50% {
    transform: translateX(100px);
    transform: translateY(200px);
  }
  100% {
    transform: translateX(233px);
  }
}

header {
  color: white;
  margin-bottom: 5.125rem;
}
.libras {
  margin-bottom: 8rem;
}

#menu {
  background-image: url("~/assets/img/vamos-viajar.png");
  background-size: contain;
  background-repeat: no-repeat;
  background-position: top;
  height: 22.52125rem;
}
#menu-mulheres,
#menu-jogo-e-mundo,
#menu-linha-do-tempo,
#menu-voce-ja-viu {
  height: 11.25rem;
  width: 11.25rem;
}
.mulheres-importantes {
  height: 11.25rem;
  width: 11.25rem;
  background: #4bd4fe;
  border-radius: 100%;
  animation-name: grow-shrink, expand, translate;
  animation-duration: 8s, 5s, 5s, 10s;
  animation-iteration-count: infinite;
}
a {
  text-decoration: none;
  color: #000000;
}

.m-shadow {
  width: 11.25rem;
  height: 11.25rem;
  border-radius: 100%;
  border-color: #4bd4fe;
  border-style: solid;
  margin-bottom: 1rem;
  margin-right: 0.75rem;
  border-width: 2px;
}

.mulheres-importantes h2 {
  font-size: 20px;
  font-weight: 900;
}
.mulheres-importantes span {
  font-weight: 500;
}
.voce-ja-viu {
  height: 11.25rem;
  width: 11.25rem;
  background: hsla(49, 100%, 53%, 1);
  border-radius: 100%;
  animation-name: grow-shrink2, expand2, translate2;
  animation-duration: 8s, 5s, 5s, 10s;
  animation-iteration-count: infinite;
}
.v-shadow {
  width: 11.25rem;
  height: 11.25rem;
  border-radius: 100%;
  border-color: hsla(49, 100%, 53%, 1);
  border-style: solid;
  margin-bottom: 1rem;
  margin-left: 0.75rem;
  border-width: 2px;
}
.voce-ja-viu h2 {
  font-size: 20px;
}
span {
  font-weight: 900;
}
span.bg {
  background: #2a0f82;
  border-radius: 20px;
  padding: 5px;
}
span.circulo {
  border: #ffffff solid 0.5px;
  font-weight: 600;
  border-radius: 50%;
}
.vc-ja-viu-titulo {
  margin-bottom: 11.0625rem;
}
.shadow-filmes {
  background-color: #4bd4fe;
  width: 18.75rem;
  height: 13.639375rem;
  border-radius: 40px;
  z-index: -1;
  right: 14px;
  top: 10px;
}
.imagem-fundo-musicas {
  height: 116.5rem;
  width: 100%;
  background: url("~/assets/img/fundo-musicas.png");
  background-size: contain;
  background-repeat: no-repeat;
}
.titulo-musica-delas {
  color: #000000;
  margin-right: 12.5rem;
  background: url("~/assets/img/fundo-musica-delas.svg");
  background-repeat: no-repeat;
  background-position: right;
}

.jogo-e-mundo {
  height: 11.25rem;
  width: 11.25rem;
  background: #7b07d6;
  border-radius: 100%;
  animation-name: grow-shrink2, expand2, translate2;
  animation-duration: 8s, 5s, 5s, 10s;
  animation-iteration-count: infinite;
}
.jogo-e-mundo h2 {
  font-size: 20px;
}
.j-shadow {
  width: 11.25rem;
  height: 11.25rem;
  border-radius: 100%;
  border-color: #7b07d6;
  border-style: solid;
  margin-top: 1rem;
  margin-left: 0.75rem;
  border-width: 2px;
}

.linha-do-tempo {
  height: 11.25rem;
  width: 11.25rem;
  background: #f7134d;
  border-radius: 100%;
  animation-name: grow-shrink2, expand2, translate2;
  animation-duration: 8s, 5s, 5s, 10s;
  animation-iteration-count: infinite;
}
.linha-do-tempo h2 {
  font-size: 20px;
}
.l-shadow {
  width: 11.25rem;
  height: 11.25rem;
  border-radius: 100%;
  border-color: #f7134d;
  border-style: solid;
  margin-top: 1rem;
  margin-left: 0.75rem;
  border-width: 2px;
}
#mulheres-importantes {
  margin-top: 17.15rem;
}

.header {
  margin-bottom: 4.6875rem;
}
.menu {
  width: 263px;
  height: 147.12px;
  background-color: aliceblue;
}
.mulheres {
  top: -20px;
}
/* .shadow-foto {
  position: absolute;
  width: 12.8125rem;
  height: 12.8125rem;
  border-radius: 100%;
  left: 200px;
  top: 70px;
  background: #2a0f82;
  z-index: -1;
} */
.shadow-foto {
  width: 12.8125rem;
  height: 12.8125rem;
  border-radius: 100%;
  background: #7b07d6;
  top: 8px;
  left: -20px;
  z-index: -1;
}
.voce-ja-viu-titulo {
  font-weight: 600;
}

/* ---------------------------------FLICKING STYLE---------------------------------*/

@media only screen and (max-width: 600px) {
  main,
  header {
    max-width: 320px;
    width: auto;
    padding-inline: 1.5rem;
    margin: 0;
  }
  .mulheres-importantes,
  .m-shadow,
  .voce-ja-viu,
  .v-shadow,
  .jogo-e-mundo,
  .j-shadow,
  .linha-do-tempo,
  .l-shadow {
    font-size: 1rem;
    width: 5rem;
    height: 5rem;
  }
}
</style>